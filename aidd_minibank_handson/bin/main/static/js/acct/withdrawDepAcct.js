$(function() {
	...
	$("#btnWithdraw").on("click", function(){
		...
		var withdrawData = {"acno" : acno,
						"txAmt" : txAmt};
		withdrawAcct(withdrawData);
	});
	...
	function retrieveAcctDetail(acno){
		...
		$.ajax({
		    ...
		    error: function (jqXHR, textStatus, errorThrown) {
				...
				$(location).attr("href", contextPath + "acct/view/withdrawAcct");
		    }
		});
		return resMsg;
	}
	
	function withdrawAcct(withdrawData){
		$.ajax({
			type : 'POST',
			url : txApiUrl+'/rest/v0.8/withdraw',
			contentType: 'application/json',
			datatype : 'json',
			...
			success: function(data, textStatus, jqXHR){
				...
				$(location).attr("href", contextPath + "acct/view/withdrawAcctResult");
		    },
		    error: function (jqXHR, textStatus, errorThrown) {
		    	if(jqXHR.status == '417' || jqXHR.status == '500'){
			    	var responseText = jqXHR.responseText;
